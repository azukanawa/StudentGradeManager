...
int ReadStudentInfo(const char *filename, struct TableScore *pTableSc) // const char* filename：教学班数据文件名；struct TableScore * pTableSc：待建立的顺序表。
{
    ...
    while (fscanf(fp, "%s %s %d %d %d %d", tem_stu.name, tem_stu.student_NO, &tem_stu.exp_score, &tem_stu.half_term_score, &tem_stu.final_score, &tem_stu.program_score) == 6) // 保证读入六个数据
    {
        tem_stu.total_score = (int)(0.2 * tem_stu.exp_score + 0.3 * tem_stu.half_term_score + 0.3 * tem_stu.final_score + 0.2 * tem_stu.program_score + 0.5); // 计算总评成绩
        ...
        pTableSc->students[pTableSc->student_count++] = tem_stu; // 将学生信息计入顺序表中
    }
    fclose(fp); // 关闭文件
    fp = NULL;  // 避免野指针
...
void output(struct TableScore *pTableSc) // 输出统计信息
{
    printf("学生信息：\n");
    printf("-----------------------------------------------------\n");
    printf("序 号  姓 名       学 号    班 级   实验成绩  半期成绩  期末成绩  项目成绩  总成绩\n");
    int index = 0;
    char class_NO[7] = {'\0'};
